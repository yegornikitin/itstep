...
while True:
    start_input = input("------------------------------\n"
                        ...
                        "Print '3' - Exit \n")
    if start_input == "1":
        input_name = input("Please, enter a name with a capital letter: ")
        if add_new_member_name(input_name) == error:
            print(error)
            continue
        input_surname = input("Please, enter a surname with a capital letter: ")
        add_new_member_surname(input_surname)
        if add_new_member_surname(input_surname) == error:
            print(error)
            continue
        input_mobile_phone = input("Please, enter a mobile in format '+380XXXXXXX' : ")
        add_new_member_mobile_phone(input_mobile_phone)
        if add_new_member_mobile_phone(input_mobile_phone) == error:
            print(error)
            continue
        input_email = input("Please, enter an email using domain'@gmail.com': ")
